...
{
public:
	representative (btcb::account account_a, btcb::amount weight_a, std::shared_ptr<btcb::transport::channel> channel_a) :
	account (account_a), weight (weight_a), channel (channel_a)
	{
	}
	std::reference_wrapper<btcb::transport::channel const> channel_ref () const
	{
		return *channel;
	};
	btcb::account account{ 0 };
	btcb::amount weight{ 0 };
...
class rep_crawler
{
	...
	mi::indexed_by<
		mi::hashed_unique<mi::member<representative, btcb::account, &representative::account>>,
		mi::random_access<>,
		...
		mi::ordered_non_unique<mi::tag<tag_weight>,
			mi::member<representative, btcb::amount, &representative::weight>, std::greater<btcb::amount>>,
		mi::hashed_non_unique<mi::tag<tag_channel_ref>,
			mi::const_mem_fun<representative, std::reference_wrapper<btcb::transport::channel const>, &representative::channel_ref>>>>;
	// clang-format on

public:
	rep_crawler (btcb::node & node_a);
	...
	void query (std::vector<std::shared_ptr<btcb::transport::channel>> const & channels_a);

	/** Attempt to determine if the peer manages one or more representative accounts */
	void query (std::shared_ptr<btcb::transport::channel> channel_a);
	 ...
	 */
	bool response (std::shared_ptr<btcb::transport::channel> channel_a, btcb::account const & rep_account_a, btcb::amount const & weight_a);

	/** Get total available weight from representatives */
	btcb::uint128_t total_weight () const;
	...
	std::vector<std::shared_ptr<btcb::transport::channel>> representative_endpoints (size_t count_a);

	/** Returns all representatives registered with weight in descending order */
	std::vector<btcb::representative> representatives_by_weight ();
...
private:
	...
	std::vector<std::shared_ptr<btcb::transport::channel>> get_crawl_targets (btcb::uint128_t total_weight_a);

	/** When a rep request is made, this is called to update the last-request timestamp. */
	void on_rep_request (std::shared_ptr<btcb::transport::channel> channel_a);
