...
TEST (wallet, construction)
{
	...
	auto wallet_l (system.nodes[0]->wallets.create (btcb::uint256_union ()));
	auto key (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key));
	wallet->start ();
...
TEST (wallet, status)
{
	...
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key.pub));
	wallet->start ();
	auto wallet_has = [wallet](btcb_qt::status_types status_ty) {
	...
	system.nodes[0]->network.udp_channels.insert (btcb::endpoint (boost::asio::ip::address_v6::loopback (), 10000), 0);
	...
	ASSERT_FALSE (wallet_has (btcb_qt::status_types::synchronizing));
	system.deadline_set (25s);
	while (!wallet_has (btcb_qt::status_types::synchronizing))
	{
	...
	ASSERT_TRUE (wallet_has (btcb_qt::status_types::disconnected));
}
...
TEST (wallet, startup_balance)
{
	...
	ASSERT_EQ ("Balance: 0 BTCB", wallet->self.balance_label->text ().toStdString ());
}
...
TEST (wallet, select_account)
{
	...
	btcb::system system (24000, 1);
	auto wallet_l (system.nodes[0]->wallets.create (btcb::uint256_union ()));
	btcb::public_key key1 (wallet_l->deterministic_insert ());
	btcb::public_key key2 (wallet_l->deterministic_insert ());
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], wallet_l, key1));
	wallet->start ();
...
TEST (wallet, password_change)
{
	...
	system.wallet (0)->insert_adhoc (btcb::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	}
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	QTest::mouseClick (wallet->settings_button, Qt::LeftButton);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
...
TEST (wallet, send)
{
	...
	btcb::public_key key1 (system.wallet (1)->insert_adhoc (btcb::keypair ().prv));
	auto account (btcb::test_genesis_key.pub);
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	wallet->start ();
	ASSERT_NE (wallet->rendering_ratio, btcb::raw_ratio);
	QTest::mouseClick (wallet->send_blocks, Qt::LeftButton);
	...
	btcb::uint128_t amount (wallet->node.balance (key1));
...
TEST (wallet, process_block)
{
	...
	btcb::system system (24000, 1);
	btcb::account account;
	btcb::block_hash latest (system.nodes[0]->latest (btcb::genesis_account));
	system.wallet (0)->insert_adhoc (btcb::keypair ().prv);
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		account = system.account (transaction, 0);
	...
	btcb::send_block send (latest, key1.pub, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, system.work.generate (latest));
	std::string previous;
...
TEST (wallet, create_send)
{
	...
	QTest::keyClicks (wallet->block_creation.account, btcb::test_genesis_key.pub.to_account ().c_str ());
	...
	bool error (false);
	btcb::state_block send (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (btcb::process_result::progress, system.nodes[0]->process (send).code);
	ASSERT_EQ (btcb::process_result::old, system.nodes[0]->process (send).code);
}
...
TEST (wallet, create_open_receive)
{
	...
	btcb::system system (24000, 1);
	system.wallet (0)->insert_adhoc (btcb::test_genesis_key.prv);
	system.wallet (0)->send_action (btcb::test_genesis_key.pub, key.pub, 100);
	btcb::block_hash latest1 (system.nodes[0]->latest (btcb::test_genesis_key.pub));
	system.wallet (0)->send_action (btcb::test_genesis_key.pub, key.pub, 100);
	btcb::block_hash latest2 (system.nodes[0]->latest (btcb::test_genesis_key.pub));
	...
	QTest::keyClicks (wallet->block_creation.representative, btcb::test_genesis_key.pub.to_account ().c_str ());
	QTest::mouseClick (wallet->block_creation.create, Qt::LeftButton);
	...
	bool error (false);
	btcb::state_block open (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (btcb::process_result::progress, system.nodes[0]->process (open).code);
	ASSERT_EQ (btcb::process_result::old, system.nodes[0]->process (open).code);
	...
	bool error2 (false);
	btcb::state_block receive (error2, tree2);
	ASSERT_FALSE (error2);
	ASSERT_EQ (btcb::process_result::progress, system.nodes[0]->process (receive).code);
	ASSERT_EQ (btcb::process_result::old, system.nodes[0]->process (receive).code);
}
...
TEST (wallet, create_change)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	btcb::state_block change (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (btcb::process_result::progress, system.nodes[0]->process (change).code);
	ASSERT_EQ (btcb::process_result::old, system.nodes[0]->process (change).code);
}
...
TEST (history, short_text)
{
	...
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), account));
	btcb::mdb_store store (init, system.nodes[0]->logger, btcb::unique_path ());
	ASSERT_TRUE (!init);
	btcb::genesis genesis;
	btcb::ledger ledger (store, system.nodes[0]->stats);
	{
		auto transaction (store.tx_begin_write ());
		store.initialize (transaction, genesis);
		btcb::keypair key;
		auto latest (ledger.latest (transaction, btcb::test_genesis_key.pub));
		btcb::send_block send (latest, btcb::test_genesis_key.pub, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, system.work.generate (latest));
		ASSERT_EQ (btcb::process_result::progress, ledger.process (transaction, send).code);
		btcb::receive_block receive (send.hash (), send.hash (), btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, system.work.generate (send.hash ()));
		ASSERT_EQ (btcb::process_result::progress, ledger.process (transaction, receive).code);
		btcb::change_block change (receive.hash (), key.pub, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, system.work.generate (receive.hash ()));
		ASSERT_EQ (btcb::process_result::progress, ledger.process (transaction, change).code);
	}
	btcb_qt::history history (ledger, btcb::test_genesis_key.pub, *wallet);
	history.refresh ();
...
TEST (wallet, startup_work)
{
	...
	{
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		ASSERT_TRUE (wallet->wallet_m->store.work_get (transaction, btcb::test_genesis_key.pub, work1));
	}
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	while (again)
	{
		ASSERT_NO_ERROR (system.poll ());
		auto transaction (system.nodes[0]->wallets.tx_begin_read ());
		again = wallet->wallet_m->store.work_get (transaction, btcb::test_genesis_key.pub, work1);
	}
}
...
TEST (wallet, import)
{
	...
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[1], system.wallet (1), key2.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, republish)
{
	...
	btcb::block_hash hash;
	{
		...
		auto latest (system.nodes[0]->ledger.latest (transaction, btcb::test_genesis_key.pub));
		btcb::send_block block (latest, key.pub, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, system.work.generate (latest));
		hash = block.hash ();
	...
	ASSERT_FALSE (system.nodes[1]->balance (btcb::test_genesis_key.pub).is_zero ());
	system.deadline_set (10s);
	while (system.nodes[1]->balance (btcb::test_genesis_key.pub).is_zero ())
	{
		ASSERT_NO_ERROR (system.poll ());
	}
}
...
TEST (wallet, ignore_empty_adhoc)
{
	...
	btcb::keypair key1;
	system.wallet (0)->insert_adhoc (key1.prv);
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1.pub));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	ASSERT_EQ (wallet->accounts.window, wallet->main_stack->currentWidget ());
	QTest::keyClicks (wallet->accounts.account_key_line, btcb::test_genesis_key.prv.data.to_string ().c_str ());
	QTest::mouseClick (wallet->accounts.account_key_button, Qt::LeftButton);
	ASSERT_EQ (1, wallet->accounts.model->rowCount ());
...
TEST (wallet, change_seed)
{
	...
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), wallet_key));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
...
TEST (wallet, seed_work_generation)
{
	...
	btcb::system system (24000, 1);
	auto key1 (system.wallet (0)->deterministic_insert ());
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system.nodes[0], system.wallet (0), key1));
	wallet->start ();
	QTest::mouseClick (wallet->show_advanced, Qt::LeftButton);
	ASSERT_EQ (wallet->advanced.window, wallet->main_stack->currentWidget ());
	QTest::mouseClick (wallet->accounts_button, Qt::LeftButton);
	...
	btcb::deterministic_key (seed.data, 0, prv);
	...
	auto transaction (system.nodes[0]->store.tx_begin_read ());
	ASSERT_FALSE (btcb::work_validate (system.nodes[0]->ledger.latest_root (transaction, pub), work));
}
...
TEST (wallet, DISABLED_synchronizing)
{
	...
	auto wallet (std::make_shared<btcb_qt::wallet> (*test_application, processor, *system0.nodes[0], system0.wallet (0), key1));
	wallet->start ();
	{
		auto transaction (system1.nodes[0]->store.tx_begin_write ());
		auto latest (system1.nodes[0]->ledger.latest (transaction, btcb::genesis_account));
		btcb::send_block send (latest, key1, 0, btcb::test_genesis_key.prv, btcb::test_genesis_key.pub, system1.work.generate (latest));
		system1.nodes[0]->ledger.process (transaction, send);
	}
	ASSERT_EQ (0, wallet->active_status.active.count (btcb_qt::status_types::synchronizing));
	system0.nodes[0]->bootstrap_initiator.bootstrap (system1.nodes[0]->network.endpoint ());
	system1.deadline_set (10s);
	while (wallet->active_status.active.count (btcb_qt::status_types::synchronizing) == 0)
	{
	...
	while (wallet->active_status.active.count (btcb_qt::status_types::synchronizing) == 1)
	{
		ASSERT_NO_ERROR (system0.poll ());
		ASSERT_NO_ERROR (system1.poll ());
		test_application->processEvents ();
