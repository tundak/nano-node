...
TEST (socket, concurrent_writes)
{
	...
	btcb::util::counted_completion read_count_completion (total_message_count);
	std::function<void(std::shared_ptr<btcb::socket>)> reader = [&read_count_completion, &total_message_count, &reader](std::shared_ptr<btcb::socket> socket_a) {
	...
	auto server_socket (std::make_shared<btcb::server_socket> (node, endpoint, max_connections, btcb::socket::concurrency::multi_writer));
	boost::system::error_code ec;
	server_socket->start (ec);
	ASSERT_FALSE (ec);
	std::vector<std::shared_ptr<btcb::socket>> connections;

	// On every new connection, start reading data
	server_socket->on_connection ([&connections, &reader](std::shared_ptr<btcb::socket> new_connection, boost::system::error_code const & ec_a) {
		if (ec_a)
		{
	...
	btcb::util::counted_completion connection_count_completion (client_count);
	std::vector<std::shared_ptr<btcb::socket>> clients;
	for (unsigned i = 0; i < client_count; i++)
	{
		auto client (std::make_shared<btcb::socket> (node, boost::none, btcb::socket::concurrency::multi_writer));
	...
	ASSERT_EQ (node->stats.count (btcb::stat::type::tcp, btcb::stat::detail::tcp_accept_success, btcb::stat::dir::in), client_count);
	// We may exhaust max connections and have some tcp accept failures, but no more than the client count
	ASSERT_LT (node->stats.count (btcb::stat::type::tcp, btcb::stat::detail::tcp_accept_failure, btcb::stat::dir::in), client_count);
