...
startup_time (std::chrono::steady_clock::now ())
{
	if (!init_error ())
	{
		...
		logger.always_log ("Node starting, version: ", BTCB_VERSION_STRING);
...
{
	assert (balance_a >= amount_a * nano::Gbcb_ratio);
	auto balance_l (balance_a);
	double result (0.0);
	for (auto i (0); i < amount_a; ++i)
	{
		balance_l -= nano::Gbcb_ratio;
		auto balance_scaled ((balance_l / nano::Mbcb_ratio).convert_to<double> ());
