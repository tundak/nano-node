...
TEST (node, mass_vote_by_hash)
{
	...
	for (auto i (0); i < 10000; ++i)
	{
		auto block (std::make_shared<nano::state_block> (nano::test_genesis_key.pub, previous, nano::test_genesis_key.pub, nano::genesis_amount - (i + 1) * nano::Gbcb_ratio, key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (previous)));
		previous = block->hash ();
...
TEST (confirmation_height, many_accounts_single_confirmation)
{
	...
	auto last_open_hash = node->latest (nano::test_genesis_key.pub);
	{
		...
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			nano::send_block send (last_open_hash, key.pub, nano::Gbcb_ratio, last_keypair.prv, last_keypair.pub, system.work.generate (last_open_hash));
...
TEST (confirmation_height, many_accounts_many_confirmations)
{
	...
	std::vector<std::shared_ptr<nano::open_block>> open_blocks;
	{
		...
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			nano::send_block send (latest_genesis, key.pub, nano::Gbcb_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (latest_genesis));
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
...
TEST (confirmation_height, long_chains)
{
	...
	nano::send_block send (latest, key1.pub, nano::genesis_amount - nano::Gbcb_ratio + num_blocks + 1, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (latest));
	...
	auto previous_destination_chain_hash = open.hash ();
	{
		...
		{
			nano::send_block send (previous_genesis_chain_hash, key1.pub, nano::genesis_amount - nano::Gbcb_ratio + i + 1, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (previous_genesis_chain_hash));
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
	...
	nano::send_block send1 (previous_destination_chain_hash, nano::test_genesis_key.pub, nano::Gbcb_ratio - 2, key1.prv, key1.pub, system.work.generate (previous_destination_chain_hash));
	auto receive1 (std::make_shared<nano::state_block> (nano::test_genesis_key.pub, previous_genesis_chain_hash, nano::genesis_account, nano::genesis_amount - nano::Gbcb_ratio + 1, send1.hash (), nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (previous_genesis_chain_hash)));

	// Unpocketed
	nano::state_block send2 (nano::genesis_account, receive1->hash (), nano::genesis_account, nano::genesis_amount - nano::Gbcb_ratio, key1.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (receive1->hash ()));
...
TEST (confirmation_height, prioritize_frontiers_overwrite)
{
...

	{
		...
		for (auto i = num_accounts - 1; i > 0; --i)
		{
			...
			nano::send_block send (last_open_hash, key.pub, nano::Gbcb_ratio - 1, last_keypair.prv, last_keypair.pub, system.work.generate (last_open_hash));
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
			nano::open_block open (send.hash (), last_keypair.pub, key.pub, key.prv, key.pub, system.work.generate (key.pub));
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, open).code);
			last_open_hash = open.hash ();
	...
	nano::keypair key;
	auto latest_genesis = node->latest (nano::test_genesis_key.pub);
	nano::send_block send (latest_genesis, key.pub, nano::Gbcb_ratio - 1, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (latest_genesis));
	...
	nano::send_block send1 (send.hash (), key.pub, nano::Gbcb_ratio - 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, system.work.generate (send.hash ()));
