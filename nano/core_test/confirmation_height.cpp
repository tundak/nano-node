...
TEST (confirmation_height, all_block_types)
{
	...
	nano::state_block epoch (key2.pub, state_change.hash (), nano::test_genesis_key.pub, nano::Gbcb_ratio, node->ledger.epoch_link (nano::epoch::epoch_1), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (state_change.hash ()));

	nano::state_block epoch1 (key1.pub, change.hash (), key2.pub, nano::Gbcb_ratio, node->ledger.epoch_link (nano::epoch::epoch_1), nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (change.hash ()));
...
TEST (confirmation_height, frontiers_confirmation_mode)
{
	...
	// Always mode
	{
		...
		auto node = system.add_node (node_config);
		nano::state_block send (nano::test_genesis_key.pub, genesis.hash (), nano::test_genesis_key.pub, nano::genesis_amount - nano::Gbcb_ratio, key.pub, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *node->work_generate_blocking (genesis.hash ()));
		{
			auto transaction = node->store.tx_begin_write ();
			ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, send).code);
		}
...
TEST (confirmation_height, dependent_election)
{
	...
	auto send = std::make_shared<nano::send_block> (latest, key1.pub, nano::genesis_amount - nano::Gbcb_ratio, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (latest));
	auto send1 = std::make_shared<nano::send_block> (send->hash (), key1.pub, nano::genesis_amount - nano::Gbcb_ratio * 2, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send->hash ()));
	auto send2 = std::make_shared<nano::send_block> (send1->hash (), key1.pub, nano::genesis_amount - nano::Gbcb_ratio * 3, nano::test_genesis_key.prv, nano::test_genesis_key.pub, *system.work.generate (send1->hash ()));
	{
		auto transaction = node->store.tx_begin_write ();
		ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send).code);
		ASSERT_EQ (nano::process_result::progress, node->ledger.process (transaction, *send1).code);
